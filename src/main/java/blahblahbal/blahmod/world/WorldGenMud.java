...
public class WorldGenMud implements IWorldGenerator
{
    ...
    public WorldGenMud(int num, Block block)
    {
    ...
    {
    	BlockPos position = new BlockPos(chunkX + 8, rand.nextInt(75-60) + 60, chunkZ + 8);
    	String s = worldIn.getBiomeGenForCoords(position).biomeName;
    	if (s.contains("Tropics"))
    	{
	        int i = rand.nextInt(this.numberOfBlocks - rand.nextInt(4) + 2) + rand.nextInt(4) + 2;
	        int j = 1;
	            ...
	            for (int l = position.getZ() - i; l <= position.getZ() + i; ++l)
	            {
	                ...
	                if (i1 * i1 + j1 * j1 <= i * i)
	                {
	                    for (int k1 = position.getY() - j; k1 <= position.getY() + j; ++k1)
	                    {
	                        ...
	                        if (block == ModBlocks.blackSand || block == ModBlocks.tropicMud)
	                        {
