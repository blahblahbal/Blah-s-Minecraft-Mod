...
public class TeleporterNetherCore extends Teleporter
{
    ...
    public TeleporterNetherCore(WorldServer par1WorldServer) {
    ...
    public void placeInPortal(Entity entityIn, float rotationYaw) {
        if (this.worldServerInstance.provider.getDimensionId() != 1) {
            ...
            int i = MathHelper.floor_double((double)entityIn.posX);
            int k = MathHelper.floor_double((double)entityIn.posZ);
            BlockPos j = this.worldServerInstance.getHeight(new BlockPos(i, 0, k));
            boolean b0 = true;
            boolean b1 = false;
            entityIn.setLocationAndAngles((double)i, (double)j.getY() + 4.0, (double)k, entityIn.rotationYaw, 0.0f);
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
            ...
            for (int i1 = -128; i1 <= 128; ++i1)
            {
                ...
                for (int j1 = -128; j1 <= 128; ++j1)
                {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.worldServerInstance.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2)
                    {
                        ...
                        /*if (this.worldServerInstance.getBlockState(blockpos1).getBlock() == ModBlocks.corePortal)
                        {
                            while (this.worldServerInstance.getBlockState(blockpos2 = blockpos1.down()).getBlock() == ModBlocks.corePortal)
                            {
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D)
        {
            ...
            /*BlockPattern.PatternHelper blockpattern$patternhelper = ModBlocks.corePortal.func_181089_f(this.worldServerInstance, blockpos);
    ...
    public boolean makePortal(Entity p_85188_1_)
    {
        ...
        for (int j2 = j - i; j2 <= j + i; ++j2)
        {
            ...
            for (int l2 = l - i; l2 <= l + i; ++l2)
            {
                ...
                {
                    if (this.worldServerInstance.isAirBlock(new BlockPos(j2, j3, l2)))
                    {
                        while (j3 > 0 && this.worldServerInstance.isAirBlock(new BlockPos(j2, j3 - 1, l2)))
                        {
                            --j3;
                        }
                            ...
                            for (int j4 = 0; j4 < 3; ++j4)
                            {
                                for (int k4 = 0; k4 < 4; ++k4)
                                {
                                    for (int l4 = -1; l4 < 4; ++l4)
                                    {
                                        ...
                                        bp = new BlockPos(i5, j5, k5);

                                        if (l4 < 0 && !this.worldServerInstance.getBlockState(bp).getBlock().getMaterial().isSolid() || l4 >= 0 && !this.worldServerInstance.isAirBlock(bp))
                                        {
                                            continue label142;
                                        }
                                    }
        ...
        if (d0 < 0.0D)
        {
            for (int l5 = j - i; l5 <= j + i; ++l5)
            {
                ...
                for (int j6 = l - i; j6 <= l + i; ++j6)
                {
                    ...
                    {
                        if (this.worldServerInstance.isAirBlock(new BlockPos(l5, i7, j6)))
                        {
                            while (i7 > 0 && this.worldServerInstance.isAirBlock(new BlockPos(l5, i7 - 1, j6)))
                            {
                                --i7;
                            }
                            ...
                            for (int k7 = i2; k7 < i2 + 2; ++k7)
                            {
                                ...
                                for (int j10 = 0; j10 < 4; ++j10)
                                {
                                    for (int j11 = -1; j11 < 4; ++j11)
                                    {
                                        ...
                                        int i13 = i7 + j11;
                                        int j13 = j6 + (j10 - 1) * j9;
                                        bp = new BlockPos(j12, i13, j13);

                                        if (j11 < 0 && !this.worldServerInstance.getBlockState(bp).getBlock().getMaterial().isSolid() || j11 >= 0 && !this.worldServerInstance.isAirBlock(bp))
                                        {
                                            continue label562;
                                        }
                                    }
        ...
        /*IBlockState iblockstate = ModBlocks.corePortal.getDefaultState().withProperty(BlockPortal.AXIS, l6 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
        ...
        for (int i8 = 0; i8 < 4; ++i8)
        {
            for (int l8 = 0; l8 < 4; ++l8)
            {
                for (int l9 = -1; l9 < 4; ++l9)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(l10, l11, k12), flag1 ? ModBlocks.netherIce.getDefaultState() : iblockstate, 2);
                }
            }
