...
public class BlahWorldGen2 implements IWorldGenerator
{
	...
	public BlahWorldGen2(Block block, Block block2, int[] dimensions, int maxY, int minY, int vpc, int opv, String biome) // , BiomeDictionary.Type biomeType)
	{
	...
	{
		if (arg3.provider.getDimensionId() == dimensions[0])
	...
	public void generateSurface(World par1World, Random par2Random, int chunkX, int chunkZ)
	{
		...
		if (s.contains(biomeName))
		{
			...
			{
				int X = chunkX * 16 + par2Random.nextInt(16);
				int Y = par2Random.nextInt(height);
				int Z = chunkZ * 16 + par2Random.nextInt(16);
