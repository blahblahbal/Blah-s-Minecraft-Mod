...
import blahmod.crafting.SmelteryRecipes;
...
public class TileInventoryFurnace extends TileEntity implements IInventory, ITickable {
	...
	private boolean smeltItem(boolean performSmelt)
	{
		...
		for (int inputSlot = FIRST_INPUT_SLOT; inputSlot < FIRST_INPUT_SLOT + INPUT_SLOTS_COUNT; inputSlot++)
		{
			if (itemStacks[inputSlot] != null)
			{
				...
				if (result != null)
				{
					//if (result.getItem() != ModItems.caesiumIngot)
	...
	{
		return SmelteryRecipes.instance().getSmeltingResult(stack);
	}
